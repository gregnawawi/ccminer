...
#include <unistd.h>
#include <assert.h>
#define VEROS_KEY_SIZE 8832
#define VEROS_KEY_SIZE128 552
#include <stdexcept>
#include <vector>
...
extern "C" inline void GenNewCLKey(unsigned char *seedBytes32, u128 *keyback)
{
	...
	int n256blks = VEROS_KEY_SIZE >> 5;  //8832 >> 5
	int nbytesExtra = VEROS_KEY_SIZE & 0x1f;  //8832 & 0x1f
	...
	u128 * g_prand, u128 *g_prandex)
{
 ...
 extern "C" inline void VerosHashHalf(void *result2, unsigned char *data, size_t len)
{
...
extern "C" void inline Veros2hash(unsigned char *hash, unsigned char *curBuf, uint32_t nonce,
	u128 *data_key, uint8_t *gpu_init, uint32_t *fixrand, uint32_t *fixrandex, u128 *g_prand, 
	u128 *g_prandex, int version)
{
	//uint64_t mask = VEROS_KEY_SIZE128; //552
	...
	intermediate = verosclhashv2_2(data_key, curBuf, 511, fixrand, fixrandex, g_prand, g_prandex);
		//FillExtra
...
extern "C" int scanhash_veros(int thr_id, struct work *work, uint32_t max_nonce, unsigned long *hashes_done)
{
	//	unsigned char data[] = {
	...
	//double secs, solps;
	u128 *data_key =  (u128*)malloc(VEROS_KEY_SIZE + 1024);

	//u128 *data_key_master = NULL;
//	posix_memalign((void**)&data_key, sizeof(__m128i), VEROS_KEY_SIZE);
	u128 *data_key_prand = data_key + VEROS_KEY_SIZE128 ;
	u128 *data_key_prandex = data_key + VEROS_KEY_SIZE128 + 32;
  //u128 data_key[VEROS_KEY_SIZE128] = { 0 }; // 552 required
	//u128 data_key_master[VEROS_KEY_SIZE128] = { 0 };
	...
	VerosHashHalf(blockhash_half, (unsigned char*)full_data, 1487);
...
Veros2hash((unsigned char *)vhash, (unsigned char *)blockhash_half, nonce_buf, data_key, 
...
// cleanup
void free_veroshash(int thr_id)
{
	if (!init[thr_id])
		return;
